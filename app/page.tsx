    ...
    <main id="main-content" className="min-h-screen">
      ...
      <section className="relative mx-4 overflow-hidden rounded-[2rem] bg-slate-950 text-white sm:mx-6"><div className="pointer-events-none absolute -right-24 -top-24 h-96 w-96 rounded-full bg-teal-500/10 blur-3xl" /><div className="relative mx-auto grid max-w-7xl gap-12 px-6 py-14 sm:px-10 lg:grid-cols-[1.2fr_.8fr] lg:gap-16 lg:py-24">
        ...
        <aside className="self-center rounded-3xl border border-white/15 bg-white/[.06] p-6 shadow-2xl sm:p-8">
          ...
          <ol className="mt-6 space-y-3">{[['01', 'Atur periode akademik', 'Tahun pelajaran dan semester', true], ['02', 'Susun data kelas', 'Tingkat dan penetapan wali kelas', true], ['03', 'Lanjutkan ke penilaian & rapor', 'Modul tahap berikutnya', false]].map(([step, title, text, ready]) => <li key={String(step)} className="flex gap-4 rounded-2xl border border-white/10 bg-slate-950/30 p-4"><span className="pt-1 text-sm font-semibold text-teal-300">{step}</span><div><p className="text-sm font-semibold">{title}</p><p className="mt-1 text-xs leading-5 text-slate-400">{text}</p><span className={`mt-2 inline-block rounded-full px-2 py-0.5 text-[11px] ${ready ? 'bg-teal-300/10 text-teal-200' : 'bg-white/10 text-slate-300'}`}>{ready ? 'Tersedia untuk admin' : 'Segera hadir'}</span></div></li>)}</ol>
        </aside>
      </div></section>
